...
import com.orange.cepheus.cep.utils.Constants;
    ...
    public void update(EventBean[] added, EventBean[] removed, EPStatement epStatement, EPServiceProvider epServiceProvider) {
        ...
        for (EventBean eventBean : added) {
            ...
            if (eventTypeOut == null) {
            	...
            	Iterator<Broker> it = eventTypeOut.getBrokers().iterator();
            	Broker ampqBroker = it.next(); // value to send to cepheus-broker to resend to an amqp broker
            	logger.debug("ampqBroker values:{}", ampqBroker.toString());
            	UpdateContextCepheus updateContextCepheus = buildUpdateContextRequest(eventBean, eventTypeOut, ampqBroker);

                if (updateContextCepheus != null) {
                    for (Broker broker : eventTypeOut.getBrokers()) {
                        assert broker != null;
                        // XXX setting a fixed value for the cepheus-broker
//                        broker.setUrl(Constants.CEPHEUS_BROKER_URL);
                        
                        logger.debug("EventBean {} , updateContext {}", eventBean.toString()
                        		,  updateContextCepheus.toString()); 
//                        logger.debug("broker.getUrl():" + broker.getUrl() 
//                        	+ " , ampqBroker.getUrlRabbitBroker():" + ampqBroker.getUrlRabbitBroker()
//                        	+ " , ampqBroker.getQueueName():" + ampqBroker.getQueueName());
                        HttpHeaders httpHeaders = getHeadersForBroker(broker);
                        // Comm. to cepheus-broker
                        ngsiClient.updateContextCepheus(broker.getUrl(), httpHeaders, updateContextCepheus).addCallback(
     ...
     */
    private UpdateContextCepheus buildUpdateContextRequest(EventBean eventBean, EventTypeOut eventTypeOut, Broker broker) {
        ContextElement contextElement = eventMapper.contextElementFromEvent(eventBean, eventTypeOut, broker);
        ...
        UpdateContextCepheus updateContextCepheus = new UpdateContextCepheus();
        updateContextCepheus.setUpdateContext(updateContext);
        updateContextCepheus.setConnectionRemoteBroker(broker.getUrlRabbitBroker());
        updateContextCepheus.setQueueName(broker.getQueueName());
        
        return updateContextCepheus;
    }
    ...
    private HttpHeaders getHeadersForBroker(Broker broker) {
        HttpHeaders httpHeaders = ngsiClient.getRequestHeaders(broker.getUrl(), null);
