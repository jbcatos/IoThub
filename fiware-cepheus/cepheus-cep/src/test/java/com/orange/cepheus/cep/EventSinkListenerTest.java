...
public class EventSinkListenerTest {
    ...
    public void postMessageOnEventUpdate() {
        ...
        when(ngsiClient.getRequestHeaders(any(), any())).thenReturn(httpHeaders);
        ...
        verify(ngsiClient).updateContextCepheus(eq(broker.getUrl()), headersArg.capture(), updateContextArg.capture());

        // Check updateContext is valid
        UpdateContextCepheus updateContext = updateContextArg.getValue();
        assertEquals(UpdateAction.APPEND, updateContext.getUpdateContext().getUpdateAction());
        ...
        ContextElement contextElement = updateContext.getUpdateContext().getContextElements().get(0);
    ...
    public void fallbackOnConfigurationId() {
        ...
        // Capture updateContext when postUpdateContextRequest is called on updateContextRequest,
        ArgumentCaptor<UpdateContextCepheus> updateContextArg = ArgumentCaptor.forClass(UpdateContextCepheus.class);
        verify(ngsiClient).updateContextCepheus(eq(broker.getUrl()), any(), updateContextArg.capture());

        // Check id correspond to the one set in configuration
        ContextElement contextElement = updateContextArg.getValue().getUpdateContext().getContextElements().get(0);
