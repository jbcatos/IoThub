...
import com.orange.cepheus.broker.model.ContextElementDTO;
...
import com.orange.ngsi.model.UpdateContextCepheus;
...
public class NgsiController extends NgsiBaseController {
    ...
    @Override
    public UpdateContextResponse updateContextCepheus(final UpdateContextCepheus updateContextCepheus) throws ExecutionException, InterruptedException, URISyntaxException {
        //TODO : search providingApplication for all contextElement of updateContext
        ContextElement contextElement = updateContextCepheus.getUpdateContext().getContextElements().get(0);
        ...
        if (providingApplication.hasNext()) {
            ...
            HttpHeaders httpHeaders = ngsiClient.getRequestHeaders(providerUrl, this.request);           
            logger.debug("=> updateContext forwarded to {} with Content-Type {}", providerUrl, httpHeaders.getContentType());
            return ngsiClient.updateContext(providerUrl, httpHeaders, updateContextCepheus.getUpdateContext()).get();
        }
        ...
        if (configuration.isRemoteForwardUpdateContext()) {
            ...
            if (brokerUrl == null || brokerUrl.isEmpty()) {
                ...
                logger.debug("=> updateContext forwarded to remote broker {} with Content-Type {} and update.toString(): {}"
                		, brokerUrl, httpHeaders.getContentType(), updateContextCepheus.getUpdateContext().toString());
                
                ngsiClient.updateContext(brokerUrl, httpHeaders, updateContextCepheus.getUpdateContext())
        ...
        for (ContextElement c : updateContextCepheus.getUpdateContext().getContextElements()) {
    ...
    @Override
    public UpdateContextResponse updateContext(final UpdateContext updateContext) throws ExecutionException, InterruptedException, URISyntaxException {
        //TODO : search providingApplication for all contextElement of updateContext
        ...
        if (providingApplication.hasNext()) {
            ...
            HttpHeaders httpHeaders = ngsiClient.getRequestHeaders(providerUrl, this.request);
            logger.debug("=> updateContext forwarded to {} with Content-Type {}", providerUrl, httpHeaders.getContentType());
            return ngsiClient.updateContext(providerUrl, httpHeaders, updateContext).get();
        }
        ...
        if (configuration.isRemoteForwardUpdateContext()) {
            ...
            if (brokerUrl == null || brokerUrl.isEmpty()) {
                ...
                HttpHeaders httpHeaders = getRemoteBrokerHeaders(brokerUrl);
                logger.debug("=> updateContext forwarded to remote broker {} with Content-Type {} and update.toString() {}"
                		, brokerUrl, httpHeaders.getContentType(), updateContext.toString());
                            
                ngsiClient.updateContext(brokerUrl, httpHeaders, updateContext)
                        .addCallback(updateContextResponse -> logUpdateContextResponse(updateContextResponse, brokerUrl),
                                throwable -> logger.warn("UpdateContext failed for {}: {}", brokerUrl, throwable.toString()));
            }
        }
        ...
        List<ContextElementResponse> contextElementResponseList = new ArrayList<>();
        StatusCode statusCode = new StatusCode(CodeEnum.CODE_200);
        for (ContextElement c : updateContext.getContextElements()) {
            contextElementResponseList.add(new ContextElementResponse(c, statusCode));
        }
    ...
    private HttpHeaders getRemoteBrokerHeaders(String brokerUrl) {
        HttpHeaders httpHeaders = ngsiClient.getRequestHeaders(brokerUrl, this.request);
    ...
    private void logUpdateResponse(String response, String rmUrl) {
    	logger.warn("Response for {}: {}", rmUrl, response.toString());
//        if (response.getStatus() != null) {
//            logger.warn("UpdateContext failed for {}: {}", brokerUrl, updateContextResponse.getErrorCode().toString());
//        } else {
//        	response.getContextElementResponses().forEach(contextElementResponse -> {
//                if (contextElementResponse.getStatusCode().getCode().equals(CodeEnum.CODE_200.getLabel())) {
//                    logger.debug("UpdateContext completed for {} ", brokerUrl);
//                } else {
//                    logger.warn("UpdateContext failed for {}: entityId {} {}", brokerUrl,
...
//        }
	...
	public String removingInternalAttributes(List<ContextElement> contextElementList) throws Exception {
		// Working and using the original array !!!
		ObjectMapper mapper = new ObjectMapper();
		SimpleModule module = new SimpleModule();
		module.addSerializer(JsonView.class, new JsonViewSerializer());
		mapper.registerModule(module);
		
		String json = mapper.writeValueAsString(JsonView.with(contextElementList)
				.onClass(ContextElement.class, match()
						.exclude("queueName")
						.exclude("connectionRemoteBroker")));
		return json;
	}
...
//        "type":"Alert",
//        "isPattern":false
	public String temporalFormatModifications(List<ContextElement> contextElementList) throws Exception {
		
		ContextElement contextElement = contextElementList.get(0);
		
		ContextElementDTO contextElementDTO = new ContextElementDTO();
		contextElementDTO.setDevice(contextElement.getEntityId().getId());
		contextElementDTO.setResources(contextElement.getContextAttributeList());
		
		// Working and usi = ng the original array !!!
		ObjectMapper mapper = new ObjectMapper();
		SimpleModule module = new SimpleModule();
		module.addSerializer(JsonView.class, new JsonViewSerializer());
		mapper.registerModule(module);
		
		String json = mapper.writeValueAsString(JsonView.with(contextElementDTO)
				.onClass(ContextElement.class, match()
						.exclude("entityId.type")
						.exclude("entityId.isPattern")));
		
//		String json = mapper.writeValueAsString(JsonView.with(contextElementDTO));
		
//		json.replace("contextAttributeList", "resources");
		return json;
	}
}
