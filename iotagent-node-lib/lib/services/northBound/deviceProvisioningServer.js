...
function handleProvision(req, res, next) {
    function handleProvisioningFinish(error, results) {
	    ...
	    /*res.status(201).json({"topic1_single_attr":topic1,"topic2_attrs":topic2,"topic3_conf_comm":topic3});*/
            var topic1="/C2NET/" + req.body.devices[0].device_id + "/attributes/+";
            var topic2="/C2NET/" + req.body.devices[0].device_id + "/attributes";
	    var topic4="/C2NET/command/request";
            var topic5="/C2NET/"+ req.body.devices[0].device_id +"/command/response";
	    res.status(201).json({"topic_single_attr":topic1,"topic_mult_attrs":topic2,"topic_command_req":topic4,"topic_command_resp":topic5});            
        }
    }
